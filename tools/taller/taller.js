...
let TITV=null, TITV_CARGANDO=false; // avisos de ITV (ver más abajo)
...
function tBarra(){
  ...
  if(T_ITV_OK()) vistas.push(["itv","Avisos ITV"+(TITV&&TITV.pendientes?` (${TITV.pendientes})`:"")]);
...
renderOrdenes=function(){
  ...
  if(TVISTA==="itv"){ $("#ordenes").innerHTML=tItv(); if(!TITV&&!TITV_CARGANDO) tItvCargar(); return; }
  $("#ordenes").innerHTML=TVISTA==="mios"?tMios():tTabla();
};

/* ---------- avisos de ITV ----------
   Solo salen los clientes que marcaron «avisarme de la ITV» en FORM-01 y tienen la fecha «ITV caduca».
   Un aviso a los 30 días y otro a los 7. El botón abre WhatsApp con el mensaje escrito: tú pulsas enviar. */
async function tItvCargar(){ TITV_CARGANDO=true; try{ TITV=await tApi("itv"); }catch(err){ TITV={error:err.message,avisos:[],pendientes:0}; } TITV_CARGANDO=false; if(TVISTA==="itv") renderOrdenes(); }
function tItvMsg(a){
  const nom=(a.nombre||"").trim().split(/\s+/)[0]||"", coche=a.coche?`tu ${a.coche}`:"tu coche", f=tF(a.itv);
  const cuando=a.dias<0?`caducó el ${f}`:a.dias===0?"caduca hoy":a.dias===1?"caduca mañana":`caduca el ${f}`;
  return `Hola${nom?" "+nom:""}, te escribimos de Volcano Cars. Te avisamos, como nos pediste, de que la ITV de ${coche} (${(a.matricula||"").toUpperCase()}) ${cuando}. `+
    (a.dias<0?"Circular con la ITV caducada puede suponer una multa. ":"")+
    `Si quieres, te hacemos una pre-ITV antes para que vayas tranquilo: dinos qué día te viene bien. Si prefieres no recibir más avisos, contéstanos «no» y no te escribimos más.`;
}
function tItvWa(a){ let d=String(a.telefono||"").replace(/[^\d]/g,""); if(d.startsWith("00")) d=d.slice(2); if(d.length===9) d="34"+d; return "https://wa.me/"+d+"?text="+encodeURIComponent(tItvMsg(a)); }
function tItv(){
  if(!TITV) return '<div class="empty">Cargando avisos de ITV…</div>';
  if(TITV.error) return `<div class="empty">${esc(TITV.error)} <button type="button" class="btn b-ghost b-sm" data-titv-recargar>Reintentar</button></div>`;
  const xs=TITV.avisos||[];
  const cab=`<p class="hint">Clientes que pidieron el aviso de ITV al dejar el coche. Pulsa <b>WhatsApp</b>: se abre con el mensaje escrito, lo envías tú y queda marcado. Primer aviso un mes antes; recordatorio una semana antes. <button type="button" class="btn b-ghost b-sm" data-titv-recargar>Actualizar</button></p>`;
  if(!xs.length) return cab+'<div class="empty">No hay avisos de ITV ahora mismo.<br><small>Aparecerán aquí cuando a un cliente que lo aceptó le falte un mes o menos para la ITV.</small></div>';
  const etq=a=>a.dias<0?`<em class="mal">Caducada hace ${-a.dias} día${a.dias===-1?"":"s"}</em>`:a.dias===0?'<em class="mal">Caduca hoy</em>':`<em class="${a.dias<=7?"mal":"sube"}">En ${a.dias} día${a.dias===1?"":"s"}</em>`;
  return cab+`<div class="t-tabla-wrap"><table class="t-tabla"><thead><tr><th>ITV</th><th>Coche y cliente</th><th>Aviso</th><th></th></tr></thead><tbody>${xs.map(a=>`
    <tr>
      <td data-l="ITV"><b class="num">${esc(tF(a.itv))}</b><small>${etq(a)}</small></td>
      <td data-l="Coche"><span class="mat">${esc((a.matricula||"").toUpperCase())}</span> <b>${esc(a.coche||"Coche")}</b><small>${esc(a.nombre)} · ${esc(a.telefono)}</small></td>
      <td data-l="Aviso">${a.enviado?`<small>✓ Enviado el ${esc(tF(a.enviado.slice(0,10)))}</small>`:`<b>${a.etapa==="7"?"Recordatorio (7 días)":"Primer aviso (30 días)"}</b>${a.anterior?`<small>1.º aviso: ${esc(tF(a.anterior.slice(0,10)))}</small>`:""}`}</td>
      <td class="t-acc">${a.enviado?"":`<a class="btn b-acc b-sm" href="${esc(tItvWa(a))}" target="_blank" rel="noopener" data-titv-wa="${esc(a.clave)}" data-titv-etapa="${a.etapa}">WhatsApp</a>`}<button type="button" class="btn b-ghost b-sm" data-tabrir="${esc(a.token)}" data-ttab="f1">Ficha</button><button type="button" class="btn b-ghost b-sm" data-titv-no="${esc(a.clave)}" title="Ya la ha pasado o no quiere más avisos">No avisar más</button></td>
    </tr>`).join("")}</tbody></table></div>`;
}
async function tItvMarcar(clave,accion,etapa){ try{ await tApi("itv","POST",{clave,accion,etapa}); }catch(err){ toast(err.message); } await tItvCargar(); }
document.addEventListener("click",e=>{
  const t=e.target;
  const w=t.closest("[data-titv-wa]"); if(w){ const c=w.dataset.titvWa, et=w.dataset.titvEtapa; setTimeout(()=>tItvMarcar(c,"enviado",et),400); toast("Abriendo WhatsApp. Queda marcado como enviado."); return; }
  const n=t.closest("[data-titv-no]"); if(n){ if(confirm("¿No volver a avisar a este cliente de esta ITV? (por ejemplo, si ya la ha pasado o no quiere avisos)")) tItvMarcar(n.dataset.titvNo,"no-avisar"); return; }
  if(t.closest("[data-titv-recargar]")){ TITV=null; renderOrdenes(); return; }
});
...
function tF1(){
  ...
  ${tSec(7,"Autorización y firma","",`<div class="t-g2"><div class="field"><label>Autoriza el diagnóstico hasta (€, IGIC incl.)</label><input class="in num" data-f="autorizoHasta" value="${tEsc(x.autorizoHasta)}" inputmode="decimal" maxlength="10" placeholder="Ej. 60"></div><label class="t-chk" style="align-self:end"><input type="checkbox" data-f="avisosWhatsApp" ${x.avisosWhatsApp?"checked":""}><span>Acepta recibir los avisos de la reparación por WhatsApp</span></label></div>
    <label class="t-chk"><input type="checkbox" data-f="avisosITV" ${x.avisosITV?"checked":""} ${dis}><span>El cliente quiere que le avisemos por WhatsApp cuando le toque la ITV (un mes antes y una semana antes). <small class="t-mut">Pregúntaselo: solo se marca si dice que sí. Rellena «ITV caduca» arriba.</small></span></label>
    <p class="t-legal">El cliente confirma que los datos, el inventario y los daños anotados son correctos. Cualquier trabajo que supere el importe autorizado se presupuesta y se aprueba antes de hacerlo. Sus datos se usan solo para gestionar esta reparación (política de privacidad en volcanocars.com).</p>
