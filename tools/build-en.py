...
TEXT = {
    ...
    "GARANTÍA DE PLAZO · MONZA CAR · GARANTÍA DE PLAZO · MONZA CAR ·": "ON-TIME PROMISE · MONZA CAR · ON-TIME PROMISE · MONZA CAR ·",
    "por día": "per day",
    "Compromiso Monza Car": "The Monza Car promise",
    ...
    "Aceptar": "Accept",
    "Acepto que Monza Car use estos datos solo para responder a mi solicitud. Puedo pedir que los borren cuando quiera.": "I agree that Monza Car may use these details only to answer my request. I can ask for them to be deleted at any time.",
    ...
    "Estado": "Condition",
    "Estamos para ayudarte con la compra, la venta, la carrocería o el mantenimiento de tu coche.": "We can help you buy or sell a car, repair bodywork or service your car.",
    ...
    "Hecho con pasión italiana en Fuerteventura": "Made with Italian passion in Fuerteventura",
    ...
    "Modelo": "Model",
    "Monza Car respondió:": "Monza Car replied:",
    "Muy bueno": "Very good",
    ...
    "Pagas al contado y nos encargamos del cambio de nombre y la entrega.": "Pay in full and we take care of the ownership transfer and handover.",
    ...
    "Recién": "Just",
    "Reparamos, pintamos y cuidamos tu coche con la pasión italiana por el motor. Y si buscas coche, te ofrecemos vehículos de ocasión revisados en nuestro propio taller, listos para rodar.": "We repair, paint and look after your car with an Italian passion for motoring. And if you're looking for a car, our used cars are checked in our own workshop and ready to drive.",
    ...
    "Taller": "Workshop",
    "Taller Monza Car": "Monza Car workshop",
    "Taller de chapa y pintura, mecánica rápida y compraventa de coches de ocasión en Antigua, Fuerteventura.": "Body repair and paint shop, quick servicing and used car sales in Antigua, Fuerteventura.",
    ...
    "llegados": "arrived",
    "meses": "months",
    "por pedir presupuesto o tasar tu coche": "for a quote or a valuation",
    "puntos": "points",
    "recientemente": "sold",
    ...
    "Mapa: Monza Car en Antigua": "Map: Monza Car in Antigua",
}
...
ATTR = {
    ...
    "Secciones": "Sections",
    "Monza Car, inicio": "Monza Car, home",
    "Mapa: Monza Car en Antigua": "Map: Monza Car in Antigua",
}

# ---------- 3. Cambios exactos en cabecera y código ----------
RAW = [
    ('<title>Monza Car · Coches de ocasión con 1 año de garantía, chapa y pintura y mecánica en Fuerteventura</title>',
     '<title>Monza Car · Used cars with a 1-year warranty, body repair and servicing in Fuerteventura</title>'),
    ('content="Monza Car: los mejores coches de ocasión en Fuerteventura, revisados en nuestro taller y con 1 año de garantía. Chapa y pintura y mecánica rápida en Antigua con cita online al momento."',
     'content="Monza Car: the best used cars in Fuerteventura, checked in our own workshop with a 1-year warranty. Body repair, paint and quick servicing in Antigua, near Caleta de Fuste, with instant online booking. We speak English."'),
    ('<html lang="es">', '<html lang="en">'),
    ('content="Monza Car · Chapa y pintura, mecánica rápida y coches de ocasión"', 'content="Monza Car · Body repair, paint, servicing and used cars in Fuerteventura"'),
    ('content="Opel Astra 1.6 115 CV (2010) por 2.999,99 €. Distribución nueva, aceite y filtros recién cambiados."',
     'content="Used cars checked in our own workshop with a 12-month warranty, in Antigua, Fuerteventura."'),
    ('<meta property="og:url" content="https://lestter4th.netlify.app/">', '<meta property="og:url" content="https://lestter4th.netlify.app/en/">\n<meta property="og:locale" content="en_GB">'),
    ('<link rel="canonical" href="https://lestter4th.netlify.app/">', '<link rel="canonical" href="https://lestter4th.netlify.app/en/">'),
    ('"url":"https://lestter4th.netlify.app/","image"', '"url":"https://lestter4th.netlify.app/en/","image"'),
    ('"description":"Chapa y pintura, mecánica rápida y compraventa de coches de ocasión revisados con 12 meses de garantía en Antigua, Fuerteventura."',
     '"description":"Body repair and paint, quick servicing and checked used cars with a 12-month warranty in Antigua, Fuerteventura."'),
    ...
    ('<span class="logo-tag">Chapa y pintura · Mecánica rápida</span>', '<span class="logo-tag">Body &amp; paint · Quick servicing</span>', 2),
    # datos de la empresa
    ...
    ('${Number(v("#s-km")).toLocaleString("es-ES")} km', '${Number(v("#s-km")).toLocaleString("en-GB")} km'),
    # coche fijo
    ...
    ('aria-label="Ver ${esc(c.marca+" "+c.modelo)}"', 'aria-label="View ${esc(c.marca+" "+c.modelo)}"'),
    ('<div class="month">Precio final · impuestos incl.</div>', '<div class="month">Final price · taxes incl.</div>'),
    ('<b style="color:var(--ink)">Estamos preparando nuevos coches.</b><br>Escríbenos por <a href="${wa("Hola Monza Car, busco un coche: ")}" target="_blank" rel="noopener">WhatsApp</a> y te avisamos en cuanto entre uno que encaje contigo.',
     '<b style="color:var(--ink)">New cars are on their way.</b><br>Message us on <a href="${wa("Hi Monza Car, I\'m looking for a car: ")}" target="_blank" rel="noopener">WhatsApp</a> and we\'ll let you know as soon as one comes in that suits you.'),
    ...
    ('alt="${esc(gal.c.marca+" "+gal.c.modelo)}, foto ${gal.i+1} de ${f.length}"', 'alt="${esc(gal.c.marca+" "+gal.c.modelo)}, photo ${gal.i+1} of ${f.length}"'),
    ('aria-label="Foto ${j+1}"', 'aria-label="Photo ${j+1}"'),
    ...
    ('"Dinos la marca."', '"Tell us the make."'),
    ('"Dinos el modelo."', '"Tell us the model."'),
    ('"Dinos el año."', '"Tell us the year."'),
    ('"Dinos los kilómetros."', '"Tell us the mileage."'),
    # mensajes de WhatsApp
    ('\\nServicios: ${p.length?p.join(", "):"sin especificar"}\\n${$("#t-msg").value?"Detalles: "+', '\\nServices: ${p.length?p.join(", "):"not specified"}\\n${$("#t-msg").value?"Details: "+'),
    ('`Hola Monza Car, quiero vender mi coche.\\n\\n', '`Hi Monza Car, I\'d like to sell my car.\\n\\n'),
    ('\\nCombustible: ${v("#s-fuel")}\\nEstado: ${estadoTxt()}\\n${v("#s-notes")?"Notas: "+v("#s-notes")+"\\n":""}\\nNombre: ${v("#s-name")}\\nTeléfono: ${v("#s-phone")}`',
     '\\nFuel: ${v("#s-fuel")}\\nCondition: ${estadoTxt()}\\n${v("#s-notes")?"Notes: "+v("#s-notes")+"\\n":""}\\nName: ${v("#s-name")}\\nPhone: ${v("#s-phone")}\\n(Customer speaks English)`'),
    ('wa("Hola Monza Car, tengo una consulta: ")', 'wa("Hi Monza Car, I have a question: ")', 2),
    ...
    ('encodeURIComponent("Monza Car Antigua Fuerteventura")', 'encodeURIComponent("Monza Car Antigua Fuerteventura")'),
]
